...
This function diagonalise $H$. $H_D$ is the diagonal form with the first half diagonal negative and the second one positive. $U$ is the fermionic transformation such that: $H=UH_{D}U\da$. \\
\end{proposition}
...
\begin{equation}
\label{eq:Majorana}
\Gamma \coloneqq \langle \vec{\alpha}\da \vec{\alpha} \rangle= \begin{pmatrix}
...
{\Gamma}^{maj} \coloneqq \langle  \vec{r}\da \vec{r} \rangle = \Omega \Gamma \Omega\da.
\end{equation}
\\
...
\end{align}
with $\nu_k\in[0,1]$ and $\epsilon_k\in[-\infty,+infty]$ where the boundary values are take with a limit. The complete calculation can be found in appendix \label{appendix:Eigenvalues-Relations}.
...
\begin{equation}
\rho=\frac{e^{-\frac{1}{2}\vec{\beta}^{\dagger}H_{D}\vec{\beta}}}{Z}=\frac{e^{-\frac{1}{2}\sum_{k=0}^{N-1}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}}{Z}.
\end{equation}
