...
This property easily translate to correlation matrices of states. For the two point correlators of a translational invariant state we have that $\langle a_j\da a_l \rangle = \langle a_{j+m}\da a_{l+m}\rangle$ and $\langle a_j a_l \rangle = \langle a_{j+m} a_{l+m}\rangle$ for every $m$. We see that the specific correlator is thus individuated just by the difference of the sites of the first and the second operators $\Delta \coloneqq l-j $ with $\Delta \in [-(N-1),N-1]$. Using this, we substitute $\langle a_j\da a_l \rangle \rightarrow \langle a\da a \rangle_{\Delta}$ and analogously $\langle a_j a_l \rangle \rightarrow \langle a a \rangle_{\Delta}$.
We consider the correlation matrix divided in the four quadrants
\begin{equation}
\Gamma = 
\begin{pmatrix}
\Gamma^{TL} & \Gamma^{TR} \\
...
\end{pmatrix}
\end{equation}
expressing $\Gamma^{TL}$  explicitly we have
\begin{equation}
\Gamma^{TL}
\begin{pmatrix}
...
If we further require the system to have PBC, we have that the parameter $\Delta$ is restricted to the range $[0,N-1]$. This is easily understood with an example. Consider for example the the specific instance of the correlator $\langle a\da a \rangle _{1-N} = \langle a_N\da a_1 \rangle$, because of the translational invariance property of the system and because of the PBC we know that $\langle a_N \da a_1 \rangle = \langle a_{N+1}\da a_{1+1} \rangle = \langle a_{1}\da a_2 \rangle = \langle a\da a \rangle _{1}$.\\
The $\Gamma^{TL}$ of the correlation matrix of the state of a translational invariant system with PBC will thus take the form
\begin{equation}
...
\end{pmatrix}.
\end{equation}
Matrix with this structure are called \textit{circulant matrices}.\\
A $N\times N$ circulant matrix is entirely characterised by its cyclic row vector defined as its first row. For $\Gamma^{TL}$ the cyclic row vector is $(\langle a\da a \rangle_{0}),\langle a\da a \rangle_{1}, \dots, \langle a\da a \rangle_{N-1}$. All the other rows of the matrix are obtained by cyclic permutation of this vector as it is clear from \ref{eq:GammaTL-Cyrculant}.
Circulant matrices are diagonalised by taking their fourier transform.\\
The Fourier transform on a linear lattice is defined as
\begin{equation}
	f_k = \frac{1}{\sqrt{N}}\sum_{j=1}^{N}e^{i\frac{2\pi}{N}kj}a_j, \qquad f_k\da = \frac{1}{\sqrt{N}}\sum_{j=1}^{N}e^{-i\frac{2\pi}{N}kj}a_j\da, 
\end{equation}
with inverse trasnsformations
\begin{equation}
	a_j = \frac{1}{\sqrt{N}}\sum_{k=1}^{N}e^{-i\frac{2\pi}{N}kj}f_k, \qquad a_j\da = \frac{1}{\sqrt{N}}\sum_{k=1}^{N}e^{i\frac{2\pi}{N}kj}f_k\da.
\end{equation}
It easy to verify that the Fourier modes obey to the CAR and are valid Dirac operators. The fermionic transformation $U_{\omega}$ that maps $\vec{\alpha}$ to the vector of fermionic operator $\vec{\phi}\da = (f_1, \dots, f_{N},f_1\da, \dots, f_{N}\da)=\vec{\alpha}\da U_{\omega}\da$ thus doesn't mix creation and annihilation operators. For this reason it takes the following block diagonal form
\begin{equation}
...
\end{pmatrix},
\end{equation}
where the fourier transform is implemented separately on the creation and annihilation operator by the matrix
\begin{equation}
...
We have said that circulant matrices are diagonalised by taking this fourier transorm. We can in fact see that  $W\Gamma^{TL}W\da$ has the form:



We now turn to express $\Gamma^{TL}$ on the basis of $\langle f\da f \rangle$. 


To simplify this expression, we exploit the translational invariance with noticing that $\langle a\da a\rangle{\Delta}=\frac{1}{N} \sum_{j=1}^{N} \langle a_j\da a_{j+\Delta} \rangle$. Thus we have that each term of $\Gamma^{TL}$ can be explicitly written in term of $f\da$ and $f$ as
\begin{equation}
\langle a\da a \rangle_{\Delta}= \frac{1}{N^2} \sum_{j}^{N} \sum_{k,k'}e^{i\frac{2\pi}{N}k'\Delta} e^{i\frac{2\pi}{N}(k-k')j}f_k\da f_{k'},
\end{equation} 
and collecting the Kronecker delta we have
\begin{equation}
\langle a\da a \rangle_{\Delta} = \frac{1}{N} \sum_{k=1}^{N} e^{i\frac{2\pi}{N}k\Delta} f_k\da f_k.
\end{equation}
...
In order to obtain its diagonal expression, we just have to consider the specific form of the submatrices $A$ and $B$ of the hamiltonian $H$. We have that for the hopping model $B$ is a matrix with all elements equal to $0$ and $A$ is a cyclic matrix with cyclic row vector $(0,\frac{1}{2}, \underbrace{0,\dots,0}_{N-3}, \frac{1}{2})$.


If we express the hopping hamiltonian \ref{eq:Hopping-Hamiltonian} in term of the Fourier modes we obtain
\begin{equation}
...
\includegraphics[width=0.7\textwidth]{figure/JLCODE3.pdf} }
  \captionof{figure}{Output of examples \ref{ex:JL-Hopping-Hamiltonian}, note how for a nearest neighbour hamiltonian, the contour is higher on the boundary of the partition.}
  

\begin{proposition}{\code{Build\_Fourier($N$)$\rightarrow U_{\omega}$}}{}
This function returns the matrix $U_{\omega}$ of dimension $2N\times 2N$.
\end{proposition}
