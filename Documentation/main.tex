...
\begin{equation}
	\label{eq:Dirac-QFH}
	\hat{H}=\frac{1}{2}\sum_{i,j=1}^{N}\left(A_{i,j}a_{i}^{\dagger}a_{j}-\bar{A}_{i,j}a_{i}a_{j}^{\dagger}+B_{i,j}a_{i}a_{j}-\bar{B}_{i,j}a_{i}^{\dagger}a_{j}^{\dagger}\right),
\end{equation}
%
...
\hat{H}=\frac{i}{2}\sum_{i,j=1}^{N}\left(h^{xx}_{i,j} x_i x_j +h^{pp}_{i,j} p_i p_j +h^{xp}_{i,j} x_i p_j +h^{px}_{i,j} p_i x_j\right)  =\frac{i}{2}\vec{r}^{\dagger}h\vec{r},%= \frac{1}{2}\left(\vec{\alpha}^{\dagger}\Omega^{\dagger}\right)\Omega H_{a}\Omega^{\dagger}\left(\Omega\vec{\alpha}\right)
\end{equation}
%
where
\begin{equation}
...
\begin{align}
\hat{H} 	&  	= \frac{1}{2}\vec{\alpha}^{\dagger}H\vec{\alpha}= \vec{\alpha}\da \Omega \da \Omega H\Omega\da \Omega \vec{\alpha}= \\
		&	=   \frac{i}{2} \vec{r}\da h \vec{r}.
\end{align}
...
\begin{align}
\hat{H} &	= \frac{i}{2}\vec{r}^{\dagger}h\vec{r} 
		= \frac{i}{2} \vec{r}^{\dagger}O O\da h O O\da \vec{r} =\\
	& 	=  \frac{i}{2}\vec{s}^{\dagger} \left( \bigoplus_{i=1}^{N} \begin{pmatrix}0 & \lambda_i \\ -\lambda_i & 0\end{pmatrix}  \right) \vec{s} 
		=  \frac{i}{2}\vec{s}^{\dagger} h_D \vec{s} 
\end{align}

That is  $\hat{H}=\frac{i}{2}\sum_{i=0}^{N-1}\lambda_{i}(\tilde{x}_{i}\tilde{p}_{i}-\tilde{p}_{i}\tilde{x}_{i})$
...
Mapping back to the Dirac representation we obtain the diagonal form of the hamiltonian in the Dirac operators representation as
 \begin{align}
 \hat{H} & =  \frac{i}{2}  \left(\vec{s}F_{xp\rightarrow xx}^{T} \Omega \right) \left( \Omega\da F_{xp\rightarrow xx} h_D F_{xp\rightarrow xx}^{T} \Omega \right) \left( \Omega\da F_{xp\rightarrow xx} \vec{s} \right) = \\
 	&	= \frac{1}{2}\sum_{k=1}^{N} \epsilon_{k} (b\da_k b_k -b_k b\da_k) = \frac{1}{2}\vec{\beta}\da H_D \vec{\beta}.
 \end{align}
...
\rho = \frac{e^{-\hat{H}}}{Z} = \frac{e^{-\frac{1}{2} \vec{\alpha}\da H \vec{\alpha}}}{Z}
\end{equation}
...
%%Bravyi2004 definisce anche GAUSSIANI ODD, però fa vedere che questi sono sempre riconducibili a Gaussiani even aumentando di uno modo il sistema, quindi guassiani even vanno bene per tutto.
...
\begin{equation}
\rho = \frac{e^{-\frac{1}{2}\vec{\beta}\da H_D \vec{\beta}}}{Z} = \frac{1}{Z}\bigotimes_{k=1}^{N}e^{-i  \epsilon_{k} (b\da_k b_k -b_k b\da_k)} .
\end{equation}
...
In section \ref{sec:Correlation-Matrix} we have seen that any f.g.s. $\rho$ can be expressed as a tensor product of single mode thermal states 
\begin{equation}
\rho = \frac{e^{-\frac{1}{2}\vec{\beta}\da H_D \vec{\beta}}}{Z} =\frac{1}{Z}\bigotimes_{k=1}^{N}e^{-i  \epsilon_{k} (b\da_k b_k -b_k b\da_k)}.
\end{equation}
for a particular choice of Dirac operators $b_i$.
Expressed in this basis the correlation matrix, as well as the density matrix of the state, are in diagonal form. \\
This tells us that for every correlation matrix $\Gamma$ there exist a fermiomic transformation $U$ such that $\Gamma$ can be brought in the diagonal form
\begin{equation}
...
\begin{align}
\epsilon_k = \ln \left(\frac{1-\nu_k}{\nu_k} \right),\\
\nu_k = \frac{1}{1+e^{\epsilon_k}}
\end{align}
...
\frac{1}{1+e^{\beta \epsilon_1}} & 0 & \dots &  & \dots & 0\\
0 & \ddots & \ddots &  &  & \vdots\\
\vdots & \ddots & \frac{1}{1+e^{\beta \epsilon_N}} \\
 &  &  & \frac{1}{1+e^{-\beta \epsilon_1}}  & \ddots & \vdots\\
\vdots &  &  & \ddots & \ddots & 0\\
0 & \dots &  &  & \dots0 & \frac{1}{1+e^{-\beta \epsilon_N}} 
\end{pmatrix}.
\end{equation}
...
\end{proposition}
\begin{proposition}{\code{Thermal\_fix\_energy($H_D,U,E$)$\rightarrow  \Gamma_{\beta(E)}, \beta(E)$}}{}
This function returns $\Gamma_{\beta(E)}$, the thermal state at inverse temperature $\beta(E)$ of the hamiltonian $H=UH_DU\da$, and $\beta(E)$ the temperature such that $\Tr{\rho_{\beta(E)}H}=E$.
Since the correct temperature is computed variationally, it prints in the terminal the precision $\Tr{\rho_{\beta(E)}H}-E$ and $\beta(E)$.
\end{proposition}
...
Consider the fermionic gaussian state $\rho=\frac{e^{-\frac{1}{2}\vec{\alpha}^{\dagger}H \vec{\alpha}}}{Z}$ and its correlation matrix $\Gamma$.
The diagonal form $\Gamma^{D}$ of correlation matrix  tells us that a basis exists in which the density matrix $\rho$ of the state can be written as the tensor product of $N$ density matrices, one for each site. If $\nu_{i\in\left[1,N\right]}$ are the positive eigenvalues of $\Gamma$ then there exists a basis such that:
\begin{equation}
...
We have that:
\begin{equation}
\mbox{Purity}(\rho)=\prod_{i=1}^{N-1}\frac{1}{\sech(\epsilon_{i})+1},
\end{equation}
...
In 2014 Chen and Vidal \cite{chen2014} introduced entanglement contour "a tool for identifyng which real-space degrees of freedom contribute, and how much, to the entanglement of a region A with the rest of the system B". We will present here a specific form of entanglement contour, for a more general treatment we refer to the original paper of Chen and Vidal.\\
...
\end{equation}
Factorising Hilbert space $\mathcal{H}_A$ in its tensor product structure $\mathcal{H}_A=\bigotimes_{j\in A}\mathcal{H}_j$, we individuate in each local Hilbert space $\mathcal{H}_j$ a site of the partition $A$.
...
We consider the state $\rho=\frac{e^{-\frac{1}{2}\vec{\alpha}^{\dagger}H \vec{\alpha}}}{Z}$, we diagonalise $H$ changing the basis to $\vec{\beta}=U^{\dagger}\vec{\alpha}$. Thus we have
\begin{equation}
\rho=\frac{e^{-\frac{1}{2}\vec{\beta}^{\dagger}H_{D}\vec{\beta}}}{Z}=\frac{e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}}{Z}.
\end{equation}
...
\begin{equation}
Z=Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}\right] =2^N\prod_{k=1}^{N}\left(\cosh\left(\frac{\epsilon_{k}}{2}\right)\right).
\end{equation}
...
\begin{equation}
\begin{aligned}
Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)} b_{x}^{\dagger}b_{y}\right]	&  =\sum_{v\in\left\{ 0,1\right\} ^{N}}\langle v|e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}b_{x}^{\dagger}b_{y}|v\rangle=\\
	&=\sum_{v\in\left\{ 0,1\right\} ^{N}}\langle v|e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}|\tilde{v}\rangle=\\
	& =\sum_{v\in\left\{ 0,1\right\} ^{N}}e^{-\frac{1}{2}\sum_{k=1}^{N}(-1)^{v_{k}+1}\epsilon_{k}}\langle v|\tilde{v}\rangle = 0
\end{aligned}
\end{equation}


\begin{equation}
Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}b_{x}b_{y}^{\dagger}\right]=0
\end{equation}

•\item $\forall x,y$

\begin{equation}
Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}b_{x}b_{y}\right]=0
\end{equation}
\begin{equation}
Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}b_{x}^{\dagger}b_{y}^{\dagger}\right]=0
\end{equation}

Thus the numerator can be explicitly written as

\begin{equation}
Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}\vec{\alpha}_{i}\vec{\alpha_{j}}^{\dagger}\right]=
\end{equation}

\begin{equation*}
=\sum_{l=1}^{2N}\sum_{m=1}^{2N}U_{i,l}U_{m,j}^{\dagger}Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}\vec{\beta}_{l}\vec{\beta_{m}}^{\dagger}\right]=
\end{equation*}

\begin{equation*}
=\sum_{l=1}^{N}U_{i,l}U_{l,j}^{\dagger}Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}b_{l}^{\dagger}b_{l}\right]+\sum_{l=1}^{N}U_{i,l+N}U_{l+N,j}^{\dagger}Tr\left[e^{-\frac{1}{2}\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}b_{l}b_{l}^{\dagger}\right]=
\end{equation*}
...
\begin{equation}
Z^{2}=\prod_{k=1}^{N}\left(2\cosh\left(\frac{\epsilon_{k}}{2}\right)\right)^{2}
\end{equation}

and

\begin{equation}
Tr\left[e^{-\sum_{k=1}^{N}\epsilon_{k}\left(b_{k}^{\dagger}b_{k}-b_{k}b_{k}^{\dagger}\right)}\right]=\prod_{k=1}^{N}\left(2\cosh\left(\epsilon_{k}\right)\right).
\end{equation}
