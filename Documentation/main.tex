...
\hat{H} = \sum_{i=0}^{N-1} [a_i\da a_{i+1} - a_i a_{i+1}\da],
\end{equation}
where $[a_{N-1}\da a_{N}-a_{N-1}a_{N}\da]=[a_{N-1}\da a_{0}-a_{N-1}a_{0}\da]$ is the periodic term for having periodic boundary conditions.\\
This is one of the simplest model of fermionic quadratic hamiltonian and it can be analytically diagonalised.\\
In order to do so, one first has to introduce the definition of Fourier modes on a linear lattice
\begin{equation}
	f_k = \frac{1}{\sqrt{N}}\sum_{j=0}^{N-1}e^{i\frac{2\pi}{N}kj}a_j, \qquad f_k\da = \frac{1}{\sqrt{N}}\sum_{j=0}^{N-1}e^{-i\frac{2\pi}{N}kj}a_j\da, 
\end{equation}
with inverse trasnsformations
\begin{equation}
	a_j = \frac{1}{\sqrt{N}}\sum_{k=0}^{N-1}e^{-i\frac{2\pi}{N}kj}f_k, \qquad a_j\da = \frac{1}{\sqrt{N}}\sum_{k=0}^{N-1}e^{i\frac{2\pi}{N}kj}f_k\da.
\end{equation}
It easy to verify that the Fourier modes obey to the CAR and are valid Dirac operators. The fermionic transformation $U_{\omega}$ that maps $\vec{\alpha}$ to the vector of fermionic operator $\vec{\omega}\da = (f_0, \dots, f_{N-1},f_0\da, \dots, f_{N-1}\da)=\vec{\alpha}\da U_{\omega}\da$ is 
\begin{equation}
...
\begin{equation}
\hat{H} = \sum_{k=0}^{N-1} \phi(k) (f_k\da f_k-f_k f_k\da),
\end{equation}
where 
\begin{equation}
\phi(k)=\cos(\frac{2\pi}{N}k).
\end{equation}
...
\begin{equation}
\hat{H}_{\omega} = \sum_{k=0}^{N-1} \phi_k (f_k\da f_k-f_k f_k\da), \qquad \hat{H} = \sum_{k=0}^{N-1} \epsilon_k (b_k\da b_k -b_k b_k\da),
\end{equation} 
...
\section{Correlation Matrix of a Translational Invariant State}
