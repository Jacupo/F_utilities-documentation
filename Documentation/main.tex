...
\begin{equation}
\rho = \frac{e^{-\frac{1}{2}\vec{\beta}\da H_D \vec{\beta}}}{Z} = \frac{1}{Z}\bigotimes_{k=1}^{N}e^{-i  \epsilon_{k} (b\da_k b_k -b_k b\da_k)} =\bigotimes_{k=1}^{N}\frac{e^{-i  \epsilon_{k} (b\da_k b_k -b_k b\da_k)}}{Z_k},
\end{equation}
...
\begin{equation}
 \frac{e^{-\frac{1}{2}\vec{\beta}\da H_D \vec{\beta}}}{Z} =\frac{1}{Z}\bigotimes_{k=1}^{N}e^{-i  \epsilon_{k} (b\da_k b_k -b_k b\da_k)}=\bigotimes_{k=1}^{N}\frac{e^{-i  \epsilon_{k} (b\da_k b_k -b_k b\da_k)}}{Z_k}.
\end{equation}
...
\begin{equation}
E_{H}(\Gamma) =\frac{1}{2} \sum_{k} \epsilon_k(\langle b_k\da b_k \rangle - \langle b_k b_k\da \rangle).
\end{equation}

\begin{proposition}{\code{Energy($\Gamma,H_D,U$)$\rightarrow E_{H}(\Gamma)$}}{}
This function returns $E_{H}(\Gamma)$ the energy of the state $\Gamma$ calculated with $H$. $H_D$ and $U$ are the output of \code{Diag\_h(H)}.
\end{proposition}
...
\subsubsection{Reduced density matrix and tensor product of fermionic gaussian states}
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{figure/JLCODE1.pdf}
	\caption{Output of examples \ref{ex:JL-Energies-Of-H}}
...
\item \code{Evolve\_gamma(M,D,U,t)$\rightarrow M_{t}$}: This function evolve for a time \code{t} (last argument) the correlation matrix $\Gamma$ (first argument), \code{D} is the Dirac Hamiltonian diagonalised, $U$ is the unitary transformation that change basis from the diagonal one to the original one. e.g. If I want to evolve the correlation matrix $M$ with $H_{a}$ for a time $t$ I would write \code{M\_{t}=Evolve\_gamma(M,Diag\_h($H_{a}$),$t$)}.
