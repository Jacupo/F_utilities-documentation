...
\end{pmatrix}.
\end{equation}
We notice that $\Gamma^{a\da a}$ is entirely characterised by its first row vector, all the other rows of the matrix are just cyclic permutation of the first vector. We call the first vector the \textit{cyclic row vector}. 
...
We have said that circulant matrices are diagonalised by taking this fourier transorm. We can in fact see that  $W\Gamma^{a \da a}W\da$ has the form:



We now turn to express $\Gamma^{a \da a}$ on the basis of $\langle f\da f \rangle$. 


To simplify this expression, we exploit the translational invariance with noticing that $\langle a\da a\rangle{\Delta}=\frac{1}{N} \sum_{j=1}^{N} \langle a_j\da a_{j+\Delta} \rangle$. Thus we have that each term of $\Gamma^{a \da a}$ can be explicitly written in term of $f\da$ and $f$ as
\begin{equation}
...
An $N \times N$ circulant matrix $C$ has the form
\begin{equation}
C=
\begin{pmatrix}
c_0		& c_1		& c_2		& \dots		& c_N-1
c_
\end{pmatrix}
\end{equation} 

It is known that circulant matrices are diagonalised by taking their Fourier transform.\\
