...
\hat{H} = \sum_{i=1}^{N} [a_i\da a_{i+1} - a_i a_{i+1}\da],
\end{equation}
where $[a_{N}\da a_{N+1}-a_{N}a_{N+1}\da] \coloneqq [a_{N}\da a_{1}-a_{N}a_{1}\da]$ is the periodic term for having periodic boundary conditions.\\
This is one of the simplest model of fermionic quadratic hamiltonian and it can be analytically diagonalised.\\
In order to obtain its diagonal expression, we just have to consider the specific form of the submatrices $A$ and $B$ of the hamiltonian $H$. We have that for the hopping model $B$ is a matrix with all elements equal to $0$ and $A$ is a circulat matrix with circulant vector $(0,\frac{1}{2}, \underbrace{0,\dots,0}_{N-3}, \frac{1}{2})$ (see appendix \ref{appendix:Circulant-Matrices}).


If we express the hopping hamiltonian \eqref{eq:Hopping-Hamiltonian} in term of the Fourier modes \eqref{eq:Fourier_a-to-f} we obtain
\begin{equation}
...
\subsubsection{Numerical diagonalisation of the hopping hamiltonian}
...
This functions return the hamiltonian $H$ if dimension $2N \times 2N$ for the hopping model. If \code{PBC=false} it return the hopping hamiltonian without the periodic term.
\end{proposition}
