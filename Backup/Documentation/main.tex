...
%Se non usi elegantbook, puoi usare minted
...
\section{Correlation matrix of translational invariant state}
...
\subsection{Eigenvalues using the properties of circulant-matrices}
...
\subsection{Eigenvalues using the Fourier transform on a linear lattice}
...
For an example of diagonalisation of translational invariant matrices see e.g. section \ref{sec:Hopping-Model}
...
\end{pmatrix}
\end{align}
As seen in section \ref{sec:Translational-Invariant-State} and appendix \ref{appendix:Circulant-Matrices}) we know that $H$ is diagonalised with a Fourier transformation.
...
For the numerical diagonalisation of the hamiltonian we used two methods, the analytical one using the Fourier modes, and the numerical one introduced in the previous section.\\
...
\subsection{Time evolution of translational invariant states}
Let us consider a translational invariant state $\Gamma$. In section \ref{sec:Translational-Invariant-States} we expressed $\Gamma$ in term of the Fourier modes $f_k\da,f_k$.
...
\section{Analytical diagonalisation of the TFI hamiltonian}
In this section we will see how to diagonalise the three hamiltonians \eqref{eq:TFI-Fermions-BC} analytically. For a complete and detailed treatment we refer to \cite{lieb1961,cabrera1987} or the more recent review \cite{suzuki2013}.\\
\subsection{Antiperiodic and periodic boundary condition fermionic hamiltonian}
...
\subsection{Open boundary condition fermionic hamiltonian}
...
\subsection{Analitical and Numerical energies}
\label{sec:JLCODE5}
\lstinputlisting{JLCODE5.jl}
...
\subsection{Degenerancy of the ground state}
\lstinputlisting{JLCODE6.jl}
...
\section{Block diagonal form of skew-symmetric matrices}
\label{appendix:Shurd-Decomposition}
Let $h$ be a $N\times N$ skew-symmetric matrix of rank $2m$, where $N\geq2m$.\\
Then there exist a $N\times N$ unitary matrix $U$ such that  \cite{horn1985}
\begin{equation}
...
\begin{equation}
a_j\da a_j =  \left(\otimes_{k=1}^{j-1}\sigma^z_k \right)\otimes \frac{\id_j-\sigma_j^z}{2}\left(\otimes_{k=j+1}^{N} \id_k \right).
\end{equation}
...
One of the consequence of this fact is that the procedure of partial tracing does not generally commute with the Jordan-Wigner mapping \cite{friis2013,friis2016}.
...
For a detailed and very well explained treatment of this question see \ref{friis2016}.
We end this section pointing out that, the fact that the well defined trace for fermionic system is not consistent with the mapping between fermions and qubits, leads to many interesting questions on entanglement in fermionic systems, see e.g.  \cite{li2001,schliemann2001,wolf2006,banuls2007,sindici2018,becher2020}
...
\end{enumerate}
 ...
 \item $a_j^{\dag}=-\bigotimes_{k=1}^{j-1}\sigma_k^z\otimes\sigma_j^+\bigotimes_{k=j+1}^N\mathbb{I}_k$
  \item $a_j^{\dag}a_j=\otimes_{k=1}^{j-1}\sigma^z_k \otimes \frac{\sigma_j^z+\id_j}{2}\otimes_{k=j+1}^{N} \id_k $
 \end{enumerate}
