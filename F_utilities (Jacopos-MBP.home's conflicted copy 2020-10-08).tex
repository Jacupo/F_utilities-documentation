...
We remark that Majorana operators labelled by $i$ correspond Dirac operators labelled by $i$. Moving between Majorana and Dirac operators does not mix modes.
...
\paragraph{Hamiltonian diagonal form with Dirac operators}
...
\paragraph{Hamiltonian diagonal form with Majorana operators}
...
As seen in subsection \ref{Diag-Ham-Dirac}, diagonalising a general f.q.h. $\hat{H}$ reduces to diagonalising the matrix $H$ of its compact form.\\
...
%%Theorem 1
\begin{theorem}{Block diagonal form of skew-symmetric matrices}{Block-Matrix}
...
\end{pmatrix} 
\end{equation}
with $\Gamma^{aa}_{i,j} \coloneqq \langle a_i a_j\rangle=-\overline{\Gamma^{a\da a\da}_{i,j}}$ and $\Gamma^{a a\da}_{i,j} \coloneqq \langle a_i a\da_j \rangle =(\id - \Gamma^{a\da a})\da_{i,j}$, where $\overline{A}$ is the conjugate of $A$.
...
In subsection \ref{sec:Correlation-Matrix} we have seen that for any f.g.s. $\rho$  there exist a fermionic transformation $U$ that diagonalise its parent Hamiltonian
...
\subsection{Correlation matrix of translational invariant state}
\label{sec:Translational-Invariant-States}
We consider a state $\rho$ of a system of $N$ sites and all its reduced density matrices $\rho_A$, where $A$ is any possible set of sites of the system. We denote with $A+m$ the set of sites $A+m=\{j=i+m | i$ i s a site of $ A\}$, that is a translation of all site of $A$ by $m$ sites. When we will assume Periodic Boundary Conditions (PBC) we will allow for translations "over the border" of the system, in the sense that when $i+m>N$ (or $i+m<1$)  we will substitute it with $i+m \rightarrow \mod(i+m-1,N+1)+1$. This is interpreted as connecting the first site with the last site of the system. Thus for PBC all translations are allowed. When we will assume Open Boundary Conditions (OBC) only translations within the system. This means that if $i\in A$ and $i+m>N$ (or $i+m<1$), then the subset $A+m$ is not an allowed subset of sites.\\
...
\end{pmatrix}.
\end{equation}
We see that $\Gamma^{a\da a}$ is the circulant matrix (see appendix \ref{appendix:Circulant-Matrices}) characterised by the circulant vector $\vec{\langle a\da a \rangle}= \left( \langle a\da a \rangle_{0}	, \langle a\da a \rangle_{1}, \langle a\da a \rangle_{2} ,\dots, \langle a\da a \rangle_{N-1} \right)$.  By the same reasoning, we see that $\Gamma^{a a\da}$ is a circulant matrix characterised by the circulant vector $\vec{\langle a a\da \rangle}= \left( \langle a a\da \rangle_{0}, \langle a a\da \rangle_{1}, \langle a a\da \rangle_{2} ,\dots, \langle a a\da \rangle_{N-1} \right)$.  Matrices $\Gamma^{a a}$ and $\Gamma^{a\da a\da}$ are circulant skew-simmetric matrices, often called \textit{skew-circulant matrices}. If $N$ is even $\Gamma^{a a}$ and $\Gamma^{a\da a\da}$ are specified by the circulant vectors $\vec{\langle a a \rangle}= \left( \langle a a \rangle_{0}	, \langle a a \rangle_{1}, \langle a a \rangle_{2} ,\dots,  \langle a a \rangle_{\frac{N}{2}-1}, 0,-\langle a a \rangle_{\frac{N}{2}-1},-\langle a a \rangle_{\frac{N}{2}-2},\dots,  \langle a a \rangle_{1} \right)$ and $\vec{\langle a\da a\da \rangle}= \left( \langle a\da a\da \rangle_{0}	, \langle a\da a\da \rangle_{1}, \langle a\da a\da \rangle_{2} ,\dots,  \langle a\da a\da \rangle_{\frac{N}{2}-1}, 0,-\langle a\da a\da \rangle_{\frac{N}{2}-1},-\langle a\da a\da \rangle_{\frac{N}{2}-2},\dots,  \langle a\da a\da \rangle_{1} \right)$.
If $N$ is odd $\Gamma^{a a}$ and $\Gamma^{a\da a\da}$ are specified by the circulant vectors $\vec{\langle a a \rangle}= \left( \langle a a \rangle_{0}	, \langle a a \rangle_{1}, \langle a a \rangle_{2} ,\dots,  \langle a a \rangle_{\frac{N-1}{2}}, -\langle a a \rangle_{\frac{N-1}{2}-1},-\langle a a \rangle_{\frac{N-1}{2}-2},\dots,  \langle a a \rangle_{1} \right)$ and $\vec{\langle a\da a\da \rangle}= \left( \langle a\da a\da \rangle_{0}	, \langle a\da a\da \rangle_{1}, \langle a\da a\da \rangle_{2} ,\dots,  \langle a\da a\da \rangle_{\frac{N-1}{2}}, -\langle a\da a\da \rangle_{\frac{N-1}{2}-1},-\langle a\da a\da \rangle_{\frac{N-1}{2}-2},\dots,  \langle a\da a\da \rangle_{1} \right)$.\\
...
Now we perform the substitutions \ref{eq:Fourier_a-to-f} in the expression of $\Gamma^{a \da a}$ and we further exploit the translational invariance ($\langle a\da a\rangle{\Delta}=\frac{1}{N} \sum_{j=1}^{N} \langle a_j\da a_{j+\Delta} \rangle$) to obtain
\begin{equation}
...
As last, we note that the Fourier transform does not mix creation and annihilation operators and can be implement directly on the vector of Dirac operators $\vec{\alpha}$ with the fermionic transformation $U_{\omega}$ that has the block diagonal form
\begin{equation}
...
This function return the fermionic transformation $U_{\omega}$ for a system of $N$ sites.
\end{proposition}
...
We learned about Hamiltonians and states. It is now the time to put this two ingredients together and finally talk about the unitary evolution of fermionic gaussian states.\\
...
\end{pmatrix}
\end{align}
As seen in subsection \ref{sec:Translational-Invariant-State} and appendix \ref{appendix:Circulant-Matrices}) we know that $H$ is diagonalised with a Fourier transformation.
...
\end{equation}
The fermionic transformation that diagonalise the Hamiltonian is $U_{\omega}$ as definied in \eqref{eq:Fermionic-Transformation-Fourier}.

\subsection{Numerical diagonalisation}
In the following code we show how to initialise and diagonalise the hopping Hamiltonian using functions of \code{F-utilities}.
...
\includegraphics[width=0.7\textwidth]{figure/JLCODE3_1.pdf} }
  \captionof{figure}{In this example we diagonalised the Hamiltonian with two different methods. Using the Fourier transform method we obtain the energies specified in \eqref{eq:Dispersion-Hopping}. These energies are both positive and negative. Using the \code{Diag\_h} method of \code{F\_utilities} we obtain just positive energies. The difference in the diagonal energies comes from the fact that \code{Diag\_h} for every eigenmode with negative energy substitute creation and annihilation operators in order to redefine the energy as positive, and then reorder the modes in order to have the energies in descending order. If we diagonalise with the Fourier transform than the ground state is obtained filling all the modes with negative energy. If we diagonalise with \code{Diag\_h} then the ground state corresponds to the empty state.}
  
    \centerline{
\includegraphics[width=1\textwidth]{figure/JLCODE3_2.pdf} }
  \captionof{figure}{Here we plot the real and imaginary part of the differences correlation matrices of the ground state computed with the two methods. On the real space basis of $a,a\da$ the ground state must be the same.}
...
\end{equation} 
where the difference in the energies are due to the fact that \code{Diag\_h} consider all the energies $\epsilon_k$ positive, thus defines $b_k = f_k\da$ and $b_k\da = f_k$ for each $k$ such that $\phi_k<0$, and then reorder the modes such that to modes with smaller $k$ correspond biggest energies.\\

\subsection{Time Evolution}

For the hopping model we analytically obtained the fermionic transformation $U_{\omega}$ that diagonalise the Hamiltonian. This allows us to give an analytical expression for the time evolution of the correlation matrix.\\
Expressing the correlation matrix $\Gamma$ in term of the operators $\vec{\phi}$ and computing the time evolution with the diagonal Hamiltonian \ref{eq:Hopping-Diagonal} we obtain
\begin{align}
\langle a_l\da a_m\rangle (t) & = \frac{1}{N}\sum_{k,k'}e^{i2(\phi_k-\phi_{k'})t}  e^{i\frac{2 \pi}{N} (k(l-x)-k'(m-y))}a_x\da a_y \\ %% f non b
\langle a_l a_m \rangle (t)     & = \frac{1}{N}\sum_{k,k'}e^{-i2(\phi_{k}+\phi_{k'})t} e^{-i\frac{2 \pi}{N} (k(l-x)+k'(m-y))}a_x a_y.
\end{align}
...
\includegraphics[width=1\textwidth]{figure/JLCODE4_1.pdf} }
  \captionof{figure}{This is the correlation matrix $\Gamma$ of a translational invariant state with exponentially decaying correlations}
  
    \centerline{
\includegraphics[width=0.7\textwidth]{figure/JLCODE4_2.pdf} }
  \captionof{figure}{The time evolution induced by the Hopping Hamiltonian of the real and imaginary part of $\langle a_1\da a_2 \rangle$ and $\langle a_1a_2\rangle$ of the translational invariant state specified in the code. }

\section{Transverse Field Ising Model}
The Hamiltonian of the Transverse Field Ising model (TFI) is written as
\begin{equation}
...
\hat{H} = -\sum_{n=1}^{N-1}(a_n\da-a_n)(a_{n+1}+a_{n+1}\da)+g_I\hat{P}(a_N\da-a_N)(a_1+a_1\da)-\cot(\theta)\sum_{n=1}^{N}(a_n\da a_n-a_n a_n\da),
\end{equation}
where $\hat{P}=\prod_{n=1}^{N}(1-2a_n\da a_n)$ is the parity operator introduced in \ref{sec:Clifford-Algebra}.\\
We notice that becuase of the presence of the parity operator $\hat{P}$, the TFI Hamiltonian cannot be directly mapped to a f.q.h.. 
Nonetheless, since the Hamiltonian $\hat{H}$ commutes with the parity operator $\hat{P}$ we can diagonalise $\hat{H}$ and $\hat{P}$ simultaneously. On the diagonal basis of $\hat{P}$ we have that $\hat{H}$ has the block diagonal form $\hat{H}=\hat{H}_{e}\oplus \hat{H}_{o}$, where $\hat{H}_{e}$, called even sector Hamiltonian, corresponds to the eigenvalue $+1$ of $\hat{P}$ and $\hat{H}_{o}$, the odd sector Hamiltonian, corresponds to the eigenvalue $-1$ of $\hat{P}$. We can than proceed to diagonalise the Hamiltonian on the two sectors independently. \\
...
A bit of confusion can raise from considering the boundary conditions. The boundary conditions of the TFI Hamiltonian do not corresponds to the boundary conditions of the fermionic Hamiltonian. In fact, let us consider the three f.q.h.
\begin{equation}
...
The different quantisation of the $k$ in the two cases is justified in \cite{lieb1961} and can be understood by intuition noting that with the first quantisation one would have $a_{N+1}=-a_1$, while with the second $a_{N+1}=a_1$. 
We can write the Hamiltonian in the more compact form 
\begin{equation}
...
We have that he energies $\epsilon_k$ of the Hamiltonian in diagonal form will be
\begin{equation}
...
It is know that, at finite dimension, with $N$ even, for the antiperiodic Ising model the ground state is in the odd sector, while for the periodic Ising model the ground state is in the even sector.
...
In the thermodynamic limits, the energy differences between the two sectors goes to zero, the ground state become degenerate. 
Here we present a code for finding the correct sector of the ground state and for verifying that as $N$ grows the energy difference between the ground state of the two sectors goes to zero.
...
We have $N=10$ so the ground state is expected to be in the GF=-1 sector. The computed energies confirm this.
  ...
  \centerline{
\includegraphics[width=0.85\textwidth]{figure/JLCODE5.pdf} }
  \captionof{figure}{Output of the code \ref{sec:JLCODE5}, we see that the energies computed with \code{F\_utilities} correspond to the one computed analytically.}
...
\includegraphics[width=0.85\textwidth]{figure/JLCODE6.pdf} }
  \captionof{figure}{The ground state of Hamiltonians \eqref{eq:TFI-Fermions-BC} for $g_F=\pm 1$ converges exponentially to the same value. The ground state of the antiperiodic and of the periodic transverse field Ising model is degenerate in the thermodynamic limit.}
...
Fermionic gaussian states can be used as a tool for benchmarking algorithms. We will see how tools developed for general quantum states, translate to the language of correlation matrices. To understand the idea behind the benchmarking, let's us take an explicit example. In the next subsection we are going to see the imaginary time evolution of a fermionic gaussian states. One usually use the imaginary time evolution of a state for computing the ground state of an Hamiltonian. Of course, in the case of f.g.s., computing the ground state is not the main purpose, as we know already how to compute it for any f.q.h.. Knowing already the exact results allow us to compare the algorithm for the imaginary time evolution with the exact results and get good insight in what we should expect in a context where the exact result is not known. \\
...
In this subsection we present the translation of some well known algorithm in the language of correlation matrices. The purpose of this subsection is not benchamarking these algorithm, but instead, to translate some important existing algorithms, so that we already the the translation of the possible building blocks of any novel and more complex experimental algorithm.
...
\end{equation}
The correlation matrix $\Gamma(\tau)$ can be easily obtained by the product rule (see subsection \ref{sec:Product-Rule}) of the initial f.g.s. $\rho$ and the thermal state $\rho_{\beta=\tau}=\frac{e^{-{H}\tau}}{\Tr{e^{-\hat{H}\tau}}}$.
...
When do we expect to be able to find a basis in which eigenstates are localised? In system with a limited entanglement structure. The idea is as follows.
...
\includegraphics[width=0.5\textwidth]{figure/Entropy_and_eigenvalues.pdf}}
  \captionof{figure}{In blue the eigenvalues $\lambda_i$ of  the reduced state $\Lambda_{\ell=16}$ of the ground state of a hopping hamiltonian with $N=500$. In orange the VonNeumann entropy $S(\lambda_i)$ of the mode associated to each eigenvalue $\lambda_i$. The total VonNeumann entropy of the partition $\Lambda_{\ell=16}$ is given by the sum of the entropy of each mode (see \ref{eq:VN_Entropy}). Since the entropy of a partition $\Lambda_{\ell}$ is bounded by $\log(\ell)$, with growing $\ell$ the added modes must have associated eigenvalues close to $0$ or $1$.}
  
  
  \centerline{\label{fig:prod-eigenvalues}
\includegraphics[width=0.5\textwidth]{figure/prod-eigenvalues.pdf} }
  \captionof{figure}{In blue the number of eigenvalues of $\Lambda_{\ell}$ that are $0$ or $1$ up to machine precision, in orange the total number of eigenvalues of $\Lambda_{\ell}$ in the ground state of a hopping hamiltonian with $N=500$. When the dimension of the partition $\ell>10$ the entanglement saturate and the the number of eigenvalues equal to $1$ or $0$ starts growing linearly.}
...
We report a schematic representation of the algorithm in figure \ref{fig:RBD-123}.
...
\begin{subfigure}{.30\textwidth}
  ...
  \label{fig:sRBD-1}
\end{subfigure}%
\begin{subfigure}{.30\textwidth}\centering
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{fig:sRBD-6}
\end{subfigure}%
\caption{Examples of $4$ and $6$ of the algorithm for reducing the bond dimension of a fermionic gaussian state.}
\label{fig:RBD-46}
\end{figure}
...
We know that for ground states of $1D$ local hamiltonians the amount of entanglement (measured by the VonNeumann entropy $S$) of any region of an MPS of bound dimension $D$ is bounded by $S \leq \log(D)$. In figure \ref{fig:entropies-RBD} we plotted the value of the entropy of different regions of the ground state of a random hamiltonian. The entropy is indeed bounded by  $S \leq \log(D)$ with $D=2^{m}$.



\begin{proposition}{\code{RBD($\Gamma$, $m$)$\rightarrow  \Gamma(m)$}}{}
Returns the correlation matrix $\Gamma(m)$ obtained reducing the bond dimension of $\Gamma$ to $m$.
\end{proposition}
...
\includegraphics[width=0.8\textwidth]{figure/Entropies-JLCODE7.pdf}}
  \captionof{figure}{The entropy of different regions of the ground state of a random hamiltonian.  The blue dots correspond to partitions of $\Gamma$ with first site at the boundary of the system and with dimension $\ell$. The orange dots correspond to partitions of $\Gamma$ with first site at the boundary of the system and with dimension $\ell$.The green dots are anologous to the blue dots but computed for the state $\Gamma(m=1)$  obtained reducing the bond dimension of $\Gamma$ to $m=1$. Red dots are anologous to the orange dots, but computed for $\Gamma(m=1)$. As expected since the Hamiltonian is random and long range, the entropy of the partitions are almost always saturate. The red dash-dotted horizontal lines represent the upper bound for the entropy of a partition (starting at the border or not respectively for $\log(D)$ and $2\log(D)$). As we can see the entropy is always bounded by  $S \leq \log(D)$ with $D=2^{m}$ as expected.} 
